#!/usr/bin/env python3
"""
TCP Reno-style sender for ECS 152A project.
"""
...
def main() -> None:
    ...
    print(
        f"Reno sender will send {total_bytes} bytes across "
    ...
    base_idx = 0 # oldest unACKed
    next_idx = 0 # next packet index
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        # until all packets are ACKed
        while base_idx < num_packets:
            # send new packets
            window_size = max(int(cwnd), 1)
            while next_idx < num_packets and (next_idx - base_idx) < window_size:
                seq_id, payload = transfers[next_idx]

                # record first send time for packets
            ...
            if ack_id > last_ack_id: # new ACK
                dup_ack_count = 0
                now = time.time()

                # mark all packets covered by this ACK
                for i in range(base_idx, num_packets):
                    if acked[i]:
                        continue

                    seq_i, payload_i = transfers[i]
                    end_seq_i = seq_i + len(payload_i)

                    if end_seq_i <= ack_id:
                        acked[i] = True

                        # record delay
                        if len(payload_i) > 0 and seq_i in send_times:
                            delay = now - send_times[seq_i]
                            delays.append(delay)
                    else:
                        break

                while base_idx < num_packets and acked[base_idx]:
                    base_idx += 1
                ...
                if dup_ack_count == 3:
                    ...
                    if base_idx < num_packets:
                        seq_rtx, payload_rtx = transfers[base_idx]
                        pkt = make_packet(seq_rtx, payload_rtx)
                        sock.sendto(pkt, addr)
...
if __name__ == "__main__":
    try:
        ...
        print(f"Reno sender hit an error: {exc}", file=sys.stderr)
        sys.exit(1)
